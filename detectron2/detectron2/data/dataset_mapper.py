...
def focused_cropping(image: np.ndarray, mask: np.ndarray, background_mask: np.ndarray, focus_crop_ratio: float):
    ...
    height = background_mask.shape[0]
    width = background_mask.shape[1]

    # Get crop coordinates
    crop_pos = (x_start, y_start, height, width)

    # Crop the mask and the image
    cropped_mask = mask[y_start:y_max+y_padding, x_start:x_max+x_padding]
    cropped_image = image[y_start:y_max+y_padding, x_start:x_max+x_padding, :]
...
def init_background_mask_from_path(background_mask_path: str):
    ...
    for img_id, pred_res in tqdm(bg_masks.items(), desc="Preparing predicted background"):
        background_mask[int(img_id)] = mask_util.decode(pred_res['pred'])
        print(background_mask[int(img_id)].shape)
        exit(0)
